...
export class RoboVac {
  ...
  statuses: StatusResponse | null = null;
  lastStatusUpdate: number | null = null;
  maxStatusUpdateAge: number = 1000 * (1 * 30); //30 Seconds
  timeoutDuration: number;

  constructor(config: RoboVacConfig, debugLog = false, timeoutDuration = 2) {
  ...
  async getStatuses(force = false): Promise<{ devId: string; dps: { [key: string]: string | boolean | number } }> {
    if (force || !this.lastStatusUpdate || (new Date()).getTime() - this.lastStatusUpdate > this.maxStatusUpdateAge) {
      return await this.doWork(async () => {
        this.statuses = await this.api.get({ schema: true });
        this.lastStatusUpdate = (new Date()).getTime();
        return this.statuses as { devId: string; dps: { [key: string]: string | boolean | number } };
      });
    } else {
      return this.statuses as { devId: string; dps: { [key: string]: string | boolean | number } };
    }
  ...
  async getCleanSpeed(force = false): Promise<CleanSpeed> {
    const statuses = await this.getStatuses(force);
  ...
  async getPlayPause(force = false): Promise<boolean> {
    const statuses = await this.getStatuses(force);
  ...
  async getWorkMode(force = false): Promise<WorkMode> {
    const statuses = await this.getStatuses(force);
  ...
  async getWorkStatus(force = false): Promise<WorkStatus> {
    const statuses = await this.getStatuses(force);
  ...
  async getBatteryLevel(force = false) {
    const statuses = await this.getStatuses(force);
    return <number>statuses.dps[this.BATTERY_LEVEL];
  }

  async getErrorCode(force = false): Promise<ErrorCode> {
    const statuses = await this.getStatuses(force);
  ...
  formatStatus() {
    if (this.statuses && this.statuses.dps) {
      console.log(`
      -- Status Start --
      ...
      console.log('Statuses or dps is null');
    }
  }
