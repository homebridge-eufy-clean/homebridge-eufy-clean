 ...
 */
export = (api: API) => {
  api.registerPlatform(PLATFORM_NAME, EufyRoboVacPlatform);
};
...
class EufyCleanRoboVacAccessory implements AccessoryPlugin {
  ...
  async getBatteryLevel(callback: CharacteristicGetCallback) {
    ...
    try {
      callback(null, await this.roboVac.getBatteryLevel());
    } catch(e) {
  ...
  async getStatusLowBattery(callback: CharacteristicGetCallback) {
    try {
      callback(null, (await this.roboVac.getBatteryLevel() < 30) ? hap.Characteristic.StatusLowBattery.BATTERY_LEVEL_LOW : hap.Characteristic.StatusLowBattery.BATTERY_LEVEL_NORMAL);
    } catch(e) {
